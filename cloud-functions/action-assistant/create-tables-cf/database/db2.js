...
function createCallsTable(conn, schema) {
  return new Promise((resolve, reject) => {
    var sql = `CREATE TABLE IF NOT EXISTS ${schema}.CALLS (iduser VARCHAR(1000) NOT NULL,conversationID VARCHAR(1000) NOT NULL,userNumber VARCHAR(1000),vgwPhoneUserPhoneNumber VARCHAR(1000),vgwIsDTMF BOOLEAN,vgwBargeInOccurred BOOLEAN,concluded BOOLEAN,PRIMARY KEY(conversationID));`;
    try {
      conn.query(sql, (err) => {
        if (err) {
          if (err.sqlcode === 4136 || err.sqlcode === -601) {
